test_that("outputs from VariableSelection() are of correct dimensions (gaussian)", {
  ...
  for (PFER_thr in c(Inf, 10)){
    for (FDP_thr in c(Inf, 0.5)){
      ...
      if ((!is.infinite(PFER_thr))&!is.infinite(FDP_thr)){
        ...
        expect_warning(VariableSelection(xdata=simul$X, ydata=simul$Y,
                                         ...
                                         FDP_thr=FDP_thr,
                                         verbose=FALSE))
        stab=suppressWarnings(VariableSelection(xdata=simul$X, ydata=simul$Y,
                                                Lambda_cardinal=nlambda, K=K,
                                                pi_list=pi_list,
                                                tau=tau, n_cat=n_cat,
                                                PFER_thr=PFER_thr,
  ...
  expect_error(VariableSelection(xdata=simul$X, ydata=simul$Y,
                                 Lambda_cardinal=nlambda, K=K,
                                 pi_list=pi_list,
                                 tau=tau, n_cat=n_cat,
                                 verbose=FALSE,
...
test_that("outputs from VariableSelection() are of correct dimensions (mgaussian)", {
  ...
  stab=VariableSelection(xdata=simul$X, ydata=Y,
                         family="mgaussian",
                         Lambda_cardinal=nlambda, K=K,
                         pi_list=pi_list,
                         tau=tau, n_cat=n_cat,
  ...
  # Checking the error messages for incompatible glmnet families
  expect_error(VariableSelection(xdata=simul$X, ydata=Y,
                                 Lambda_cardinal=nlambda, K=K,
                                 pi_list=pi_list,
                                 tau=tau, n_cat=n_cat,
                                 verbose=FALSE,
