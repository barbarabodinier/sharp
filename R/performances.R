...
#' by comparing the set of selected features to the
#' set of true predictors/edges. This function can
#' only be used in simulation studies (i.e. when the
#' true model is known).
#'
#' @param theta binary vector of selected variables
#' (in a regression framework)
#' or binary adjacency matrix (in graphical modelling).
#' @param theta_star binary vector of true predictors
#' (in a regression framework)
#' or true binary adjacency matrix (in graphical modelling).
#' @param pk vector encoding the grouping structure.
#' Only used for multi-block stability selection.
#' For this, the nodes in "theta" have to be ordered
#' by group and argument "pk" has to be a vector
#' indicating the number of variables
#' in each of the groups (see example below).
#' If pk=NULL, overall performances are reported.
...
#'
#' Block-specific performances are reported if "pk" is not NULL.
#' In this case, the first row of the matrix corresponds
#' to the overall performances, and subsequent rows correspond
#' to each of the blocks.
#' The order of the blocks is defined as in \code{\link{BlockStructure}}.
#'
...
#'
#' Generates an igraph object representing the
#' True Positive, False Positive and False Negative
#' edges by comparing the set of selected edges to the
#' set of true edges. This function can
#' only be used in simulation studies (i.e. when the
#' true model is known).
#'
#' @param theta binary vector of selected variables
...
#' (in a regression framework)
#' or true binary adjacency matrix (in graphical modelling).
#' @param colours vector of edge colours.
#' The first entry of the vector defines the colour
#' of False Positive edges, second entry is for True Negatives
#' and third entry is for True Positives.
#' @param lty vector of line types for edges.
#' The first entry of the vector defines the colour
#' of False Positive edges, second entry is for True Negatives
#' and third entry is for True Positives.
#' @param plot logical indicating if the generated graph
#' should be plotted.
...
#'
#' # Performance graph
#' perfgraph=SelectionPerformanceGraph(theta=Adjacency(stab), theta_star=simul$theta, plot=TRUE)
#'
#' # User-defined colours/shapes
#' perfgraph=SelectionPerformanceGraph(theta=Adjacency(stab), theta_star=simul$theta, plot=TRUE,
#' colours=c("forestgreen", "orange", "black"), node_colour="red", node_shape="star")
#' perfgraph=SelectionPerformanceGraph(theta=Adjacency(stab), theta_star=simul$theta, plot=TRUE,
